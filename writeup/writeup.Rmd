...
```{r fig2, fig.cap="Road-sector fuel consumption per capita in Sweden, 1960--2005, measured in kilograms of oil equivalent."}
...
```{r fig3, fig.cap="Per-capita CO2 emissions from transport in Sweden and the OECD donor-sample average, 1960--2005."}
...
```{r fig4, fig.cap="Path plot of per-capita CO2 emissions from transport in Sweden and synthetic Sweden, 1960--2005."}
...
```{r fig5, fig.cap="Gap in per-capita CO2 emissions from transport between Sweden and synthetic Sweden, 1960--2005."}
...
The point estimate is $\hat{\alpha} = `r round(coef(twfe)["treated"], 3)`$ metric tons CO~2~ per capita per year across all four SE specifications.
...
The TWFE estimate ($\hat{\alpha} = `r round(coef(twfe)["treated"], 3)`$) is smaller in absolute magnitude than the synthetic control ATE ($`r round(ate_synth$ATE, 3)`$).
...
```{r fig-es, fig.cap="Event-study estimates for Sweden relative to 1989. The shaded band shows 95\\% confidence intervals from heteroskedasticity-robust standard errors."}
...
The estimates range from near zero ($n = 7,\, 8$) to `r round(filter(rdit_results, n==5)$estimate, 3)` ($n = 5$), with no stable pattern.
...
```{r fig-rdit, fig.cap="Observed CO2 emissions in Sweden with degree-7 and degree-9 piecewise polynomial fits."}
plot_rdit(sweden_ts)
```
...
The synthetic control provides the most credible point estimate (`r round(ate_synth$ATE, 3)`) and Andersson's permutation-based inference is the most appropriate framework for a single treated unit.
